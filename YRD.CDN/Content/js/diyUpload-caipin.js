...
(function( $ ) {
        ...
        diyUpload:function( opt, serverCallBack ) {
			...
			webUploader.on('error', function( code ) {
				...
				switch( code ) {
					...
					break;
					case  'F_EXCEED_SIZE' : text = '文件大小超过限制，每张图片最大为2M!';
					break;
	...
	function removeLi ( $li ,file_id ,webUploader) {
		...
		if ( $li.siblings('li').length <= 0 ) {
			...
			$('#caipin-test').find('.webuploader-pick').text('图片上传');
		} else {
			$li.remove();
		}
	...
	//继续上传
	$('#caipin-test').on('click','input[type="file"]',function(){
		$('#caipin-test').find('.webuploader-pick').text('继续上传');
	});
...
/*
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			$cancelButton.bind('click',function(){
				...
				$('#caipin-box').find('.help-inline.file').css({'display':'inline-block','marginTop':'7px'});
			});
...
*/
		...
		var $height = $('.fileBoxUl>li').length<=5 ? 140 : $('.fileBoxUl>li').length<=10 ? 280 : 'auto';
		var $maxWidth = $fileInput.parent().width();
		$width = $maxWidth > $width ? $width : $maxWidth;
		$parentFileBox.width( $width );
		$parentFileBox.height( $height );
		$parentFileBox.children('.diyButton').css('marginTop',$height+15);
		$parentFileBox.find('.viewLarge:eq(4)').css('left','-250px');
		$parentFileBox.find('.viewLarge:eq(9)').css('left','-250px');
		//$parentFileBox.siblings('.help-inline.file').css({'display':'block','marginTop':50});
		...
		webUploader.makeThumb( file, function( error, dataSrc ) {
			if ( !error ) {	
				...
				$fileBox.find('.viewLarge').append('<img src="'+dataSrc+'" >');
			}
		});

		//绑定显示大图事件
		$fileBox.on('hover','.viewThumb>img',function(){
			$fileBox.children('.viewLarge').toggle();
		});
