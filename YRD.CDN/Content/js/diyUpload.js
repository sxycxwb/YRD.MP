...
(function( $ ) {
	...
	function removeLi ( $li ,file_id ,webUploader) {
		...
		if ( $li.siblings('li').length <= 0 ) {
			...
			$('#test').find('.webuploader-pick').text('图片上传');
		} else {
			$li.remove();
		}
	...
	//继续上传
	$('#test').on('click','input[type="file"]',function(){
		$('#test').find('.webuploader-pick').text('继续上传');
	});
...
/*
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			$cancelButton.bind('click',function(){
				...
				//$('#box').find('.help-inline.file').css({'display':'inline-block','marginTop':'7px'});
			});
